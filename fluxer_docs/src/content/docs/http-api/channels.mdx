...
| thread_metadata? | ?[thread metadata](#thread-metadata-object) object | The thread state, present only for a thread channel |
| member_count? | integer | Approximate member count, present only for a thread channel |
| message_count? | integer | Approximate message count, present only for a thread channel |
| pinned? | boolean | Whether the forum post is pinned to the top of its forum |
| available_tags? | array[[forum tag](#forum-tag-object) object] | The tags a post in this forum may apply (max 20), present only for a forum channel |
| applied_tags? | array[snowflake] | The tag IDs this forum post applies (max 5), present only for a post under a forum channel |
| default_reaction_emoji? | ?object | The reaction shown by default on posts in this forum, carrying `emoji_id` and `emoji_name`, exactly one of which is null |
| default_sort_order? | ?integer | The default post ordering for a forum channel, where `0` is latest activity and `1` is creation |
| default_auto_archive_duration? | ?integer | The auto-archive duration in minutes that new posts in this forum inherit |
| default_forum_layout? | ?integer | The default post layout, where `0` is not set, `1` is list, `2` is gallery |
| default_thread_rate_limit_per_user? | ?integer | The slowmode in seconds a new post inherits |
| require_tag? | boolean | Whether a post in this forum must apply at least one tag |
| recent_participant_ids? | ?array[snowflake] | The most recent distinct message authors in a thread, newest first, up to five |
| starter_message_preview? | ?[starter message preview](#starter-message-preview-object) object | A preview of the opening message, on thread list responses only |
...
| ping_endpoint | ?string | The URL a client pings to measure latency to the region, or null when the operator configured none |
...
Thread metadata carries the archive and lock state a thread channel owns. A channel that is not a thread does not carry it.
...
| auto_archive_duration | integer | Minutes of inactivity before the thread auto-archives, one of 60, 1440, 4320, or 10080 |
| archive_timestamp? | ?ISO8601 timestamp | When the archive state last changed |
| locked? | boolean | Whether the thread is locked, so that only a moderator can unarchive it |
| invitable? | boolean | Whether a non-moderator can add others to a private thread |
| create_timestamp? | ?ISO8601 timestamp | When the thread was created |
...
```json
{
  ...
  "archive_timestamp": "2026-09-06T21:04:11.615Z",
  "locked": false,
  "invitable": false,
  "create_timestamp": "2026-09-06T21:04:11.615Z"
}
```

## Forum tag object

A forum tag is one selectable label a forum channel defines in `available_tags`. A post under that forum applies tags by ID through `applied_tags`.
...
| name | string | The name shown for the tag |
| emoji_name | ?string | The emoji shown beside the tag, or null when it has none |
| moderated | boolean | Whether only a member who can moderate threads may apply or remove this tag |

### Example

```json
{
  "id": "1472454332581490688",
  "name": "Bug report",
  "emoji_name": "🐛",
  "moderated": false
}
...
## Starter message preview object

A starter message preview summarises the opening message of a thread so a client can render a post
list without fetching each post's first message. It is present only on the thread list responses,
and only when the caller holds `READ_MESSAGE_HISTORY` on the parent channel.

| Field | Type | Description |
| --- | --- | --- |
| message_id | snowflake | The ID of the starter message |
| author | ?object | The author, carrying `id`, `username`, `global_name`, and `avatar`, or null when unresolved |
| content | string | The opening message, truncated to 200 characters, markdown untouched |
| first_attachment | ?object | The first attachment, carrying `id`, `filename`, `url`, `proxy_url`, `content_type`, `width`, and `height` |

## Thread member object

A thread member records one user's membership of a thread. [List thread members](#list-thread-members) returns these, and their count is what a thread reports as `member_count`.

### Structure

| Field | Type | Description |
| --- | --- | --- |
| user_id | snowflake | The user this membership belongs to |
| join_timestamp | ISO8601 timestamp | When the user joined the thread |
| flags | integer | Membership flags stored for the user on this thread |

### Example

```json
{
  "user_id": "1472410036750807040",
  "join_timestamp": "2026-09-06T21:04:11.615Z",
  "flags": 0
}
```
...
<RouteHeader method="POST" path="/v1/channels/{channel_id}/threads" bot />

Creates a thread under a guild text or forum channel and returns the new [channel object](#channel-object).

### Limitations

- The parent must be a guild text channel or a guild forum channel.
- The caller needs [SEND_MESSAGES](/http-api/permissions/) in the guild that owns the parent.
- `applied_tags` is accepted only when the parent is a forum channel, and every ID must appear in that forum's `available_tags`.
- A forum channel that requires a tag rejects a post that applies none.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the parent text or forum channel |

### JSON body

| Field | Type | Description |
| --- | --- | --- |
| name | string | The name of the thread, 1 to 100 characters |
| message_id? | snowflake | The message to start the thread from. The thread adopts this ID so the message can render an inline link to it |
| applied_tags? | array[snowflake] | Forum tag IDs to apply, at most 5, for a forum parent only |
| auto_archive_duration? | integer | Minutes before auto-archiving: 60, 1440, 4320, or 10080. Defaults to the parent forum's default, then 1440 |
| type? | integer | 11 for a public thread or 12 for a private thread, defaulting to 11. A private thread starts invitable |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 201 | [channel](#channel-object) object | Thread was created, or the thread that already adopted `message_id` was returned unchanged |
| 400 | [error response](/http-api/#error-response) | `applied_tags` names an undefined tag, or the parent is not a forum, each returning `FORUM_TAG_INVALID` |
| 400 | [error response](/http-api/#error-response) | The parent forum requires a tag and none was applied, returning `FORUM_TAG_REQUIRED` |
| 403 | [error response](/http-api/#error-response) | Caller lacks `SEND_MESSAGES`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Parent does not exist, is deleted, or is neither a text nor a forum channel, each returning `UNKNOWN_CHANNEL` |
| 404 | [error response](/http-api/#error-response) | `message_id` names no message in the parent channel, returning `UNKNOWN_MESSAGE` |

:::note[Starting a thread from a message is idempotent]
When `message_id` already has a thread under the same parent, that thread is returned and nothing is created or dispatched. An adopted ID that resolves to a channel which is not a thread under this parent returns `UNKNOWN_CHANNEL`.
:::

### Side effects

Fluxer emits [Thread Create](/gateway/events/#thread-create) to the sessions that can view the thread, which for a private thread means its members and anyone holding `MANAGE_CHANNELS` on the parent. The creator joins the thread before the event is emitted, so `member_count` starts at 1. For a public thread Fluxer also posts a thread-created system message in the parent channel on a best-effort basis, and a failure to post it does not fail the request; a private thread announces nothing in the parent.
...
<RouteHeader method="GET" path="/v1/channels/{channel_id}/threads" bot />

Returns the [channel objects](#channel-object) for the threads under a text or forum channel that are not archived. The array can be empty.

### Limitations

- The caller needs [VIEW_CHANNEL](/http-api/permissions/) in the guild that owns the parent.
...
| Status | Body | Condition |
| --- | --- | --- |
| 200 | array[[channel](#channel-object) object] | Threads were returned |
| 403 | [error response](/http-api/#error-response) | Caller lacks `VIEW_CHANNEL`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Parent does not exist, is deleted, or is not a guild channel, each returning `UNKNOWN_CHANNEL` |
...
<RouteHeader method="GET" path="/v1/channels/{channel_id}/threads/archived" bot />

Returns the [channel objects](#channel-object) for the archived threads under a text or forum channel. It takes the same access as [List active threads](#list-active-threads) and returns the complement of that list. The array can be empty.
...
| Status | Body | Condition |
| --- | --- | --- |
| 200 | array[[channel](#channel-object) object] | Archived threads were returned |
| 403 | [error response](/http-api/#error-response) | Caller lacks `VIEW_CHANNEL`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Parent does not exist, is deleted, or is not a guild channel, each returning `UNKNOWN_CHANNEL` |
...
<RouteHeader method="PATCH" path="/v1/channels/{channel_id}/thread" bot />

Updates a thread and returns the changed [channel object](#channel-object). Every field is optional, and an omitted field keeps its stored value.

### Limitations

- `channel_id` must name a thread, not the parent that holds it.
- The caller must own the thread, or hold [MANAGE_CHANNELS](/http-api/permissions/) in the guild.
- `applied_tags` is validated against the parent forum's `available_tags`, so it is accepted only when the thread hangs under a forum channel.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread to update |

### JSON body

| Field | Type | Description |
| --- | --- | --- |
| name? | string | New thread name, 1 to 100 characters |
| archived? | boolean | Whether the thread is archived |
| locked? | boolean | Whether the thread is locked |
| auto_archive_duration? | integer | Minutes of inactivity before auto-archiving, one of 60, 1440, 4320, or 10080 |
| invitable? | boolean | Whether non-moderators can add others to a private thread |
| applied_tags? | array[snowflake] | Replacement set of forum tag IDs, at most 5. An empty array clears the tags |
| pinned? | boolean | Whether the forum post is pinned to the top |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 200 | [channel](#channel-object) object | Thread was updated |
| 400 | [error response](/http-api/#error-response) | `applied_tags` names an undefined tag, or the thread's parent is not a forum, returning `FORUM_TAG_INVALID` |
| 403 | [error response](/http-api/#error-response) | Caller neither owns the thread nor holds `MANAGE_CHANNELS`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel does not exist, is deleted, or is not a thread, each returning `UNKNOWN_CHANNEL` |

### Side effects

Fluxer emits [Thread Update](/gateway/events/#thread-update) to every session subscribed to the guild. Changing `archived` also restamps the thread's archive timestamp, and a request that sets `archived` to the value it already holds leaves that timestamp alone.
...
<RouteHeader method="DELETE" path="/v1/channels/{channel_id}/thread" bot />

Deletes a thread. The caller must own the thread, or hold [MANAGE_CHANNELS](/http-api/permissions/) in the guild.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread to delete |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 204 | empty | Thread was deleted |
| 403 | [error response](/http-api/#error-response) | Caller neither owns the thread nor holds `MANAGE_CHANNELS`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel does not exist, is deleted, or is not a thread, each returning `UNKNOWN_CHANNEL` |

### Side effects

Fluxer emits [Thread Delete](/gateway/events/#thread-delete) to every session subscribed to the guild. That Dispatch carries only the thread's identifiers and type, not the complete channel.
...
<RouteHeader method="PUT" path="/v1/channels/{channel_id}/thread-members/@me" bot />

Adds the authenticated user to a thread. The call is idempotent.

### Limitations

- Joining a thread the caller does not already belong to needs [VIEW_CHANNEL](/http-api/permissions/) in the guild.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread to join |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 204 | empty | Caller joined the thread, or already belonged to it |
| 403 | [error response](/http-api/#error-response) | Caller lacks `VIEW_CHANNEL`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel does not exist, is deleted, or is not a thread, each returning `UNKNOWN_CHANNEL` |

:::note[An existing membership returns before the permission check]
A caller who already belongs to the thread receives 204 even when they no longer hold `VIEW_CHANNEL`, because the membership check runs first.
:::

### Side effects

A join that adds a membership recounts the thread's `member_count` and emits [Thread Members Update](/gateway/events/#thread-members-update) carrying the caller in `added_members`. A join by a user who already belongs to the thread changes nothing and emits no Dispatch.
...
<RouteHeader method="DELETE" path="/v1/channels/{channel_id}/thread-members/@me" bot />

Removes the authenticated user from a thread. The call is idempotent and needs no permission beyond naming a thread that exists.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread to leave |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 204 | empty | Caller left the thread, or did not belong to it |
| 404 | [error response](/http-api/#error-response) | Channel does not exist, is deleted, or is not a thread, each returning `UNKNOWN_CHANNEL` |

### Side effects

A leave that removes a membership recounts the thread's `member_count` and emits [Thread Members Update](/gateway/events/#thread-members-update) carrying the caller in `removed_member_ids`. A leave by a user who does not belong to the thread changes nothing and emits no Dispatch.
...
<RouteHeader method="GET" path="/v1/channels/{channel_id}/thread-members" bot />

Returns the [thread member objects](#thread-member-object) for a thread.

### Limitations

- The caller needs [VIEW_CHANNEL](/http-api/permissions/) in the guild that owns the thread.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread whose members are returned |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 200 | array[[thread member](#thread-member-object) object] | Members were returned |
| 403 | [error response](/http-api/#error-response) | Caller lacks `VIEW_CHANNEL`, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel does not exist, is deleted, or is not a thread, each returning `UNKNOWN_CHANNEL` |
...
<RouteHeader method="PUT" path="/v1/channels/{channel_id}/thread-members/{user_id}" bot />

Adds another user to a thread. The call is idempotent.

### Limitations

- The caller needs to be the thread's owner, or hold [MANAGE_THREADS](/http-api/permissions/) or `MANAGE_CHANNELS` on the parent channel.
- A member of a private thread may also add others when the thread's `thread_metadata.invitable` is true, and a member of a public thread may add others when they can send messages in it.
- The target must hold `VIEW_CHANNEL` on the parent channel.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread to add the user to |
| user_id | snowflake | The ID of the user to add |
...
| Status | Body | Condition |
| --- | --- | --- |
| 204 | empty | User was added, or already belonged to the thread |
| 403 | [error response](/http-api/#error-response) | Caller may not add this user, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel is unknown, deleted, or not a thread, each returning `UNKNOWN_CHANNEL` |
| 404 | [error response](/http-api/#error-response) | Target cannot view the parent channel, returning `UNKNOWN_MEMBER` |

### Side effects

An add that creates a membership recounts the thread's `member_count` and emits [Thread Members Update](/gateway/events/#thread-members-update) carrying the target in `added_members`.

### Rate limit

20 requests per 10 seconds for each authenticated user and channel ID, on the `channel:thread:member:add::channel_id` bucket.

## Remove a thread member

<RouteHeader method="DELETE" path="/v1/channels/{channel_id}/thread-members/{user_id}" bot />

Removes a user from a thread. Removing the caller is the same as [leaving](#leave-a-thread).

### Limitations

- Removing another user needs to be the thread's owner, or hold [MANAGE_THREADS](/http-api/permissions/) or `MANAGE_CHANNELS` on the parent channel.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the thread to remove the user from |
...
| Status | Body | Condition |
| --- | --- | --- |
| 204 | empty | User was removed, or did not belong to the thread |
| 403 | [error response](/http-api/#error-response) | Caller may not remove this user, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel does not exist, is deleted, or is not a thread, each returning `UNKNOWN_CHANNEL` |

### Side effects

A removal that deletes a membership recounts the thread's `member_count` and emits [Thread Members Update](/gateway/events/#thread-members-update) carrying the target in `removed_member_ids`.

### Rate limit

20 requests per 10 seconds for each authenticated user and channel ID, on the `channel:thread:member:remove::channel_id` bucket.

## Get a thread member

<RouteHeader method="GET" path="/v1/channels/{channel_id}/thread-members/{user_id}" bot />

Returns one [thread member object](#thread-member-object).

### Limitations

- The caller needs to be able to view the thread, which for a private thread means membership of it or thread moderation on the parent channel.
...
| user_id | snowflake | The ID of the user whose membership is returned |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 200 | [thread member](#thread-member-object) object | Membership was returned |
| 403 | [error response](/http-api/#error-response) | Caller cannot view the thread, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel is unknown, deleted, or not a thread, each returning `UNKNOWN_CHANNEL` |
| 404 | [error response](/http-api/#error-response) | User does not belong to the thread, returning `UNKNOWN_MEMBER` |

### Rate limit

40 requests per 10 seconds for each authenticated user and channel ID, on the `channel:thread:member:get::channel_id` bucket.
## Add an attachment to a forum post

<RouteHeader method="POST" path="/v1/channels/{channel_id}/starter-message/attachments" bot />

Appends an attachment that already exists on a message in this forum post to the post's starter message, where it becomes the post thumbnail.

### Limitations

- The caller needs to be the post's owner, or hold [MANAGE_THREADS](/http-api/permissions/) or `MANAGE_CHANNELS` on the parent forum.
- The source message must belong to this post, and the attachment must belong to that message.
- The starter message holds at most `max_attachments_per_message` attachments.

### Path parameters

| Field | Type | Description |
| --- | --- | --- |
| channel_id | snowflake | The ID of the forum post |

### JSON parameters

| Field | Type | Description |
| --- | --- | --- |
| message_id | snowflake | A message in this post that carries the attachment |
| attachment_id | snowflake | The attachment to append to the starter message |

### Response

| Status | Body | Condition |
| --- | --- | --- |
| 200 | [channel](#channel-object) object | The post, with its refreshed `starter_message_preview` |
| 400 | [error response](/http-api/#error-response) | The attachment is already on the starter message, or the starter message is full |
| 403 | [error response](/http-api/#error-response) | Caller may not edit this post, returning `MISSING_PERMISSIONS` |
| 404 | [error response](/http-api/#error-response) | Channel is unknown or not a forum post, returning `UNKNOWN_CHANNEL` |
| 404 | [error response](/http-api/#error-response) | Message or attachment is unknown, returning `UNKNOWN_MESSAGE` |

### Side effects

The starter message emits [Message Update](/gateway/events/#message-update). When the post's thumbnail changes, the post also emits [Thread Update](/gateway/events/#thread-update).

### Rate limit

10 requests per 10 seconds for each authenticated user and channel ID, on the `channel:thread:starter:attachment::channel_id` bucket.
