...
impl SessionInner {
    ...
    async fn create_sender(
        ...
        if track.kind() == TrackKind::Video {
            ...
            // ECHOWIRE FORK DEVIATION - do not drop the codecs we did not pick.
            //
            // Upstream calls set_codec_preferences(matched), which restricts the m-line to a single
            // codec. The SFU negotiates its media engine from the first video offer on a connection
            // and will not add codecs during a later renegotiation, so every subsequent publish is
            // answered with that first codec no matter what the new m-line offered. The track is
            // then declared as one codec and sent as another, the SFU logs "could not find codec
            // for webrtc receiver", forwards nothing, and reports no error to anyone: viewers
            // subscribe successfully and see a black tile forever. Observed live 2026-08-22, where
            // a share negotiated h264, restarted as vp8, and was invisible for the rest of the call.
            //
            // The JS SDK appends the unmatched codecs, and this restores that: preference order is
            // unchanged (the chosen codec still comes first, so it is still what we publish), the
            // rest merely stay on the offer so the SFU negotiates a complete codec set up front.
