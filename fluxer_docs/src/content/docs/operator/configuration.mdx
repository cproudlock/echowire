...
Three background jobs keep threads tidy: one archives threads that have gone quiet past their
auto-archive duration, one sweeps up threads whose parent channel was deleted before that cascade
existed, and one fills in membership index rows written before that index existed. All three run on
either value of `FLUXER_DATABASE_BACKEND`. They page every guild and read that guild's channels,
which both backends serve, so the work scales with the number of guilds rather than with the size
of the key-value table.
