...
`.env.example` `echowire`. The sender name. An empty value sets an empty sender name.
...
:::caution[Thread maintenance requires the Postgres backend]
Two background jobs keep threads tidy: one archives threads that have gone quiet past their
auto-archive duration, and one sweeps up threads whose parent channel was deleted before that
cascade existed. Both scan the key-value table directly and run only when
`FLUXER_DATABASE_BACKEND` is `postgres`. On a Cassandra deployment they exit immediately, so
threads never auto-archive there and must be archived by hand. Access control is unaffected: a
thread whose parent is gone is denied on every path regardless of backend.
:::
